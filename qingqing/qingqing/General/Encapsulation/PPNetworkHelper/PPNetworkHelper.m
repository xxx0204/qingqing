...
+ (NSURLSessionTask *)uploadImagesWithURL:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (NSUInteger i = 0; i < images.count; i++) {
            ...
            [images[i] drawInRect:CGRectMake(0,0,images[i].size.width,images[i].size.height)];
            UIImage* newImage = UIGraphicsGetImageFromCurrentImageContext();
            UIGraphicsEndImageContext();
            
            // 图片经过等比压缩后得到的二进制文件
            NSData *imageData = UIImageJPEGRepresentation(newImage, imageScale ?: 1.f);
 ...
 */
+ (void)initialize {
    _sessionManager = [AFHTTPSessionManager manager];//[PPNetworkHelper sharedHttpSessionManager];
    _sessionManager.requestSerializer = [AFJSONRequestSerializer serializer];//loginsessionid
    _sessionManager.responseSerializer = [AFJSONResponseSerializer serializer];
    NSDictionary *dict = @{ NSHTTPCookieName : @"loginSessionId",
                            NSHTTPCookieValue : [NSString memberLoginSessionId],
                            NSHTTPCookieDomain : @"",
                            NSHTTPCookiePath : @"/",
                            NSHTTPCookieVersion : @"0",
                            NSHTTPCookieExpires : [NSDate dateWithTimeIntervalSinceNow:60*60*24*360]
                            };
    NSHTTPCookie *cookie = [[NSHTTPCookie alloc] initWithProperties:dict];
    NSLog(@"cookie:%@",cookie);
    NSArray *cookies=[NSArray arrayWithObjects:cookie,nil];
    NSDictionary *headers=[NSHTTPCookie requestHeaderFieldsWithCookies:cookies];
    [_sessionManager.requestSerializer setValue:[headers objectForKey:@"Cookie"] forHTTPHeaderField:@"Cookie"];
